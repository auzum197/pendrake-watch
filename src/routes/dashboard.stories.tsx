...
function yearOfTxs(): Tx[] {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    list.push({
      ...
      status: "confirmed",
      notes: [
        { pool: "orchard", direction: kind, outputIndex: 0, valueZat: value },
      ],
    });
  }
  return [
...
const meta = {
  ...
  beforeEach: ({ args }) => {
    ...
    mocked(getSpotPrice).mockResolvedValue(
      args.price === "none" ? null : SPOT[args.price],
    );
    mocked(getPriceHistory).mockResolvedValue(
      args.price === "none" ? [] : priceHistory(),
    );
  },
  argTypes: {
    switching: {
      control: "boolean",
      description: "Another Wallet is being selected",
    },
    sync: {
      control: "select",
      options: [
    ...
    balance: {
      control: "select",
      options: ["loading", "empty", "small", "large"],
    },
    history: { control: "select", options: ["none", "few", "year"] },
    fiatEnabled: {
      control: "boolean",
      description: "USD consent already given",
    },
...
export const LongHistory: Story = {
  args: { balance: "large", history: "year" },
};
...
export const Usd: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(canvas.getByText("updated just now")).toBeVisible(),
    );
  },
};
...
export const UsdStale: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(canvas.getByRole("button", { name: "USD" }));
    await waitFor(() =>
      expect(canvas.getByText("updated 3h ago")).toBeVisible(),
    );
  },
};
...
export const UsdConsent: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(
        within(document.body).getByText("Show balances in USD?"),
      ).toBeVisible(),
    );
  },
};
