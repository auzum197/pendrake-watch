...
describe("resolveIndexer", () => {
  it("takes a mainnet preset as-is", () => {
    expect(resolveIndexer(PRESET, "https://ignored", "mainnet")).toBe(PRESET);
  });
...
describe("indexerReady", () => {
  ...
  it("needs a URL-shaped custom entry", () => {
    ...
    expect(indexerReady(CUSTOM_INDEXER, "https://mine:443", "mainnet")).toBe(
      true,
    );
  });
  ...
  it("holds regtest to the same URL shape, preset or not", () => {
    ...
    expect(indexerReady(PRESET, "http://localhost:8232", "regtest")).toBe(true);
  });
});
