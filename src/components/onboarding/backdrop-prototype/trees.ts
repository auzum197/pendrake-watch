// PROTOTYPE: procedural silhouette trees for the onboarding backdrop.
// Coordinates are in the source image's pixel space (1717 x 916).
...
type Branch = {
  ...
  width: number;
  angle: number;
  // Stiffer near the trunk, looser at the tips.
  flex: number;
  phase: number;
  leaves: Leaf[];
  children: Branch[];
...
export type TreeSpec = {
  ...
  lean: number;
  seed: number;
  // Tall trees keep a straight leader with short side shoots. Round ones fork evenly.
  habit: "tall" | "round";
  depth: number;
  foliage: number;
};
...
export function plant(spec: TreeSpec): Tree {
  ...
  // The leader shrinks by roughly r per level, so its length is a geometric sum.
...
// Placed over where the painted trees were.
...
// Wind is a slow sway, a faster shiver, and a gust that rolls in every so often
// and travels across the scene from left to right.
...
export function draw(
  ...
  // Canvas state changes are the slow part, so strokes are grouped by width
  // and leaves by tone. Leaves stay as fillRect, which beats a Path2D of
  // thousands of tiny rects.
